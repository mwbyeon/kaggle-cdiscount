...
def inception_resnet_stem(name, data,
                          ...
                          num_5_1=192,
                          bn_mom=0.9):
    stem_3x3 = Conv(data=data, num_filter=num_1_1, kernel=(3, 3), stride=(2, 2), name=('%s_conv' % name), bn_mom=bn_mom,
                    workspace=256)
    stem_3x3 = Conv(data=stem_3x3, num_filter=num_1_2, kernel=(3, 3), name=('%s_stem' % name), suffix='_conv',
                    bn_mom=bn_mom, workspace=256)
    ...
    stem_1_1x1 = Conv(data=concat1, num_filter=num_3_1, name=('%s_stem_1' % name), suffix='_conv_2', bn_mom=bn_mom,
                      workspace=256)
    stem_1_3x3 = Conv(data=stem_1_1x1, num_filter=num_3_2, kernel=(3, 3), name=('%s_stem_1' % name), suffix='_conv_3',
                      bn_mom=bn_mom, workspace=256)

    stem_2_1x1 = Conv(data=concat1, num_filter=num_4_1, name=('%s_stem_2' % name), suffix='_conv_1', bn_mom=bn_mom,
                      workspace=256)
...
def ReductionA(name, data,
               ...
               bn_mom=0.9):
    ra1 = mx.sym.Pooling(data=data, kernel=(3, 3), stride=(2, 2), pad=(0, 0), pool_type='max',
                         name=('%s_%s_pool1' % ('max', name)))
...
def ReductionB(name, data,
               ...
               bn_mom=0.9):
    rb1 = mx.sym.Pooling(data=data, kernel=(3, 3), stride=(2, 2), pad=(0, 0), pool_type='max',
                         name=('%s_%s_pool1' % ('max', name)))
...
def get_symbol(num_classes, dropout_ratio=0.2, use_squeeze_excitation=False, ratio=1/16, scale=True, **kwargs):
    ...
    pool = mx.sym.Pooling(data=in5c, global_pool=True, kernel=(5, 5), stride=(1, 1), pad=(0, 0), pool_type="avg",
                          name="global_pool")
    ...
    if dropout_ratio is not None and dropout_ratio > 0.0:
        flatten = mx.sym.Dropout(data=flatten, p=dropout_ratio)  # original
